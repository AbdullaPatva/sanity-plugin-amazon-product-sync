...
export class AmazonClient {
   ...
   */
  private createCanonicalRequest(operation: string, payload: any, headers: Record<string, string>, date: string): string {
   ...
   */
  private createAuthorizationHeader(signature: string, date: string): string {
    const scope = `${date}/${this.awsRegion}/${this.service}/aws4_request`
   ...
   */
  private transformProduct(item: any, asin: string): AmazonProduct {
    const product: AmazonProduct = { asin }
